<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ option('app_direction', 'ltr') }}" class="bg-[#F5F5F5]">
<head>
    <meta charset="utf-8">
...
{{--    <title>{{ config('app.name', 'Spack') }}</title>--}}
    ...
    <script>
        window.Config = @json(new Admin\Support\AppConfig)
    </script>

    {{ vite_tags('vendor/admin', 5173) }}
</head>
