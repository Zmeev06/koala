...
<template>
  <div class="p-2 w-full modal-content">
    <div class="bg-white rounded-[10px] w-full h-full">
      <div class="rounded-lg flex justify-center h-full py-28" v-if="task.loading">
      ...
      <div class="h-full flex flex-col" v-else>
        <div class="flex shadow-sm rounded-t-lg border-gray-200" :style="{'background-color': task.data.project ? task.data.project.meta.color : '#666666'}">
          <div class="flex items-center text-white px-6">
            ...
            <span>{{ task.data.project ? task.data.project.name : 'Новая задача' }}</span>
            <span class="px-2">/</span>
            <span>{{ task.data.project ? task.data.project_list.name : 'Новая задача' }}</span>
          </div>
          <span class="text-white block ml-auto hover:text-gray-400 cursor-pointer px-4 py-4" @click="$router.push({name: 'ProjectsDetail', params: {id: props.id}})">
