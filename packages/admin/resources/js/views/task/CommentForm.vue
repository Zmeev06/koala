<template>
  <div class="absolute bottom-0 w-full bg-white">
    <div
      class="ring-1 ring-gray-300 w-full rounded-[10px] cursor-pointer mt-2 hover:bg-gray-50"
      v-show="isSkelton"
      @click="showForm"
    >
    ...
    <form
      class="border border-gray-300 rounded-[10px]"
    ...
    >
      ...
      <ul class="mt-1 mb-1 mx-3 border rounded-md divide-y" v-if="files.length">
        <li class="block py-2 px-3" v-for="(file, index) in files">
          ...
          <div
            class="bg-green-100 w-full rounded-md mt-2"
            v-if="progress[index]"
          >
            <span
              class="block bg-green-400 rounded-md h-1"
      ...
      <div class="flex items-center pt-2 pb-3 px-3">
        <div class="flex items-center">
          <button
            type="button"
            class="inline-flex items-center px-2.5 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-0"
            @click="create"
          >
            {{ __("Save") }}
          </button>
