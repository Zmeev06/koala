<template>
  <v-dropdown name="task-repeat" :modal="true" close-outside :close="closeDropdown" @toggle="onOpen">
    ...
    <div class="bottom-0 ltr:origin-bottom-right rtl:origin-bottom-left absolute ltr:right-0 rtl:left-0 mb-6 w-96 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-40" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
      <div class="px-6 py-4">
        <h3 class="font-medium text-gray-700 text-sm mb-6">Recurrence</h3>

        <section class="flex items-center">
          <p class="whitespace-nowrap text-sm text-gray-600 mr-6">Repeat every</p>
        ...
        <section class="flex items-center mt-6" v-if="form.type == 'daily'">
          <p class="whitespace-nowrap text-sm text-gray-600 mr-6">Hour At</p>
        ...
        <section class="mt-6" v-if="form.type == 'weekly'">
          <p class="whitespace-nowrap text-sm text-gray-600">Repeat on</p>
        ...
        <section class="flex items-center mt-6" v-if="form.type == 'monthly'">
          <p class="whitespace-nowrap text-sm text-gray-600 mr-6">Day</p>
        ...
        <section class="flex items-center mt-6" v-if="form.type == 'yearly'">
          <p class="whitespace-nowrap text-sm text-gray-600 mr-6">Month / Day</p>
        ...
        <section class="mt-6">
          <p class="whitespace-nowrap text-sm text-gray-600 mb-3">Ends</p>
          ...
          <div class="space-y-1.5">
            <div class="flex items-center h-[1.875rem]">
              ...
              <label for="ends-never" class="ml-3">
                <span class="block text-sm text-gray-600">Never</span>
              </label>
            </div>
            <div class="flex items-center">
              <div class="flex items-center w-[9.375rem]">
                ...
                <label for="ends-on" class="ml-3">
                  <span class="block text-sm text-gray-600">On</span>
                </label>
              </div>
            ...
            <div class="flex items-center">
              <div class="flex items-center w-[9.375rem]">
                ...
                <label for="ends-after" class="ml-3">
                  <span class="block text-sm text-gray-600">After</span>
                </label>
              </div>

              <div>
                <div class="flex rounded-md shadow-sm border border-gray-300" :class="{ 'opacity-50': form.ends != 'after' }">
                  <input type="number" v-model="form.repetitions" class="focus:ring-0 border-0 block w-20 px-2.5 py-1.5 text-xs rounded-md text-gray-800" aria-describedby="price-currency" :disabled="form.ends != 'after'" @change="form.repetitions = Math.max(Math.min(Math.round(form.repetitions), 999), 1)">

                  <div class="flex items-center pointer-events-none pr-2.5">
                    <span class="text-gray-600 text-xs"> Occurences </span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section class="mt-8">
          ...
          <div class="flex items-center">
            ...
            <label for="task-completion" class="pl-3 text-sm text-gray-600">Only if last task was completed</label>
          </div>
        </section>
