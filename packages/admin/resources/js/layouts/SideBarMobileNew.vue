<template>
  <div
    class="md:hidden z-20 absolute bg-[#1d1b20] pt-[8px] px-[12px] h-screen transition-all duration-300"
    :class="isOpen ? 'min-w-max  w-[70%] sm:w-[50%]' : 'w-[40px]'"
  >
    <div class="" :class="isOpen ? 'absolute min-w-[40px] w-full' : 'w-[16vw]'">
      ...
      <div class="flex flex-col gap-[22px] mt-[30px]">
        <RouterLink
          :to="`/${item.link}`"
          custom
          ...
          >
            ...
            <div class="w-[80%] overflow-hidden">
             ...
             ${currentRouteName === item.id ? 'text-[#31B492]' : 'text-[#fff]'} 
              ${isOpen ? 'translate-x-0' : 'translate-x-[-100%] opacity-0'}
            `"
                class="w-full text-md duration-300 select-none"
              >
                {{ item.name }}
              </p>
            </div>
        ...
        <SidebarProjectsArchived :isOpen="isOpen" />
      </div>
    </div>
  </div>
</template>
